...
def runtmx2ea(tmxdir: str):
    tmxFiles = []

    for root, dirs, files in os.walk(tmxdir):
        for file in files:
    ...
    for tmxFilename in tmxFiles:
        eventFilename = os.path.splitext(tmxFilename)[0]+".event"
        dmpFilename   = os.path.splitext(tmxFilename)[0]+"_data.dmp"

        try:
            process(tmxFilename, eventFilename, dmpFilename, True)
            processedFiles.append(eventFilename)

        except Tmx2EaError as e:
            sys.exit("ERROR: {}: {}".format(tmxFilename, e.message))

    installerFile = tmxdir + "\\_MasterMapInstaller.event"
